...
- **RQ1.** Does a realistic protocol — temporal split combined with same-IPC hard negatives — change measured demand-prediction performance relative to a conventional random split? *(Answered in §5.x: yes, decisively — a random split lowers the patent cold-start rate from 91.7% to 2.4% and makes matrix factorization the single best method (NDCG@10 = 0.442, AUC = 0.830, vs MostPop 0.219), the opposite of the temporal verdict where the same model collapses to 0.037.)*
...
**근거/메모:** 모든 수치 FACTS 일치. **📘IPM 저널 인용 요건 충족** — Introduction에 대상 저널(*Information Processing & Management*) 게재 논문 **4편**을 직접 인용: [Abbasiantaeb et al., 2025](특허 인용/참조 마이닝, ¶2), [Boratto et al., 2021](인기편향 디바이싱, ¶3), [Deldjoo et al., 2021](데이터 특성↔정확도/공정성, ¶3), [Boratto et al., 2023](consumer fairness 완화 한계, ¶3). 모두 REFERENCES.md [58]–[61]에 검증 완료(날조 0). 저자 할 일: ① 남은 `[domain ref]` (기술이전 선행연구 일부) 보강, ② RQ1 답에 random-split 대조 수치 추가, ③ 섹션 번호(2~8)를 최종 목차에 맞춰 조정, ④ 인용 스타일(IPM은 보통 numbered)로 §3 기존 [1] 식과 통일.
...
**Single chronological cutoff.** We use a single 70/15/15 temporal split, and do not perform rolling-origin (expanding-window) evaluation, so the stability of the diagnosis across cutoffs is unverified. We do, however, report a controlled random-split contrast: re-evaluating every model under a shuffled 70/15/15 split lowers the patent cold-start rate from 91.7% to 2.4% and inflates both ranking and AUC — most strikingly, SVD rises from NDCG@10 = 0.037 (AUC ≈ 0.52) to 0.442 (AUC = 0.830), overtaking the most-popular baseline (0.219) and becoming the best model overall. The contrast confirms that the realistic protocol is what exposes the failure: a random split, by suppressing cold-start, manufactures the appearance of a competent learned model.
