...
are losses to a more-popular hard negative; score--popularity $\rho$ up to 1.0)
...
score optimized by the training loss (\S\ref{sec:data}, Models) and re-ranked by the
...
recency of their most recent training transfer, scoring more recent acquirers higher
--- ), two structural heuristics
...
and were subsequently cleaned and processed by the authors. The processed dataset can
be made available upon reasonable request by email, subject to KIPRIS terms of use.
The preprocessing and evaluation code, the experiment scripts, and a script to
regenerate the frozen SBERT embeddings from patent text are released to enable full
reproduction of all tables and figures from the KIPRIS exports.
