...
class BaseGAT(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels, dropout):
        super().__init__()
        # 이종 그래프에서는 self loop 추가가 불가능하므로 add_self_loops=False 적용
...
class FullGATModel(torch.nn.Module):
    def __init__(self, metadata, hidden_channels=32, out_channels=16, dropout=0.1):
        ...
        base_gnn = BaseGAT(hidden_channels, out_channels, dropout)
...
for idx, seed in enumerate(SEEDS):
    print(f"\n{'='*50}")
    print(f"[{idx+1}/5] Starting GAT Evaluation for Seed: {seed}")
    print(f"{'='*50}")
    ...
    # 모델 & 옵티마이저 초기화
    model = FullGATModel(train_data.metadata(), hidden_channels=HIDDEN_CHANNELS, out_channels=HIDDEN_CHANNELS//2, dropout=DROPOUT).to(device)
...
print("\n" + "="*50)
print("🎉 Final 5-Seed GAT Results 🎉")
print("="*50)
