...
# 튜닝으로 찾은 하이퍼파라미터 + DropEdge
HIDDEN_CHANNELS = 32
LEARNING_RATE = 0.01
DROPOUT = 0.1
DROP_EDGE_P = 0.2  # 무작위로 20%의 간선을 제외하는 Data Augmentation
PATIENCE = 5
MAX_EPOCHS = 100
...
###############################################################################
# 3. 모델 정의 (DropEdge 데이터 증강 적용)
###############################################################################
class BaseGNN(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels, dropout, drop_edge_p):
        super().__init__()
    ...
    def forward(self, x, edge_index):
        # Data Augmentation: DropEdge 적용 (Training 시에만 작동)
        edge_index_drop, _ = dropout_edge(edge_index, p=self.drop_edge_p, training=self.training)
        
        x = self.conv1(x, edge_index_drop).relu()
        x = F.dropout(x, p=self.dropout, training=self.training)
        
        # 두 번째 레이어에서도 독립적인 구조적 변형을 주기 위해 한 번 더 DropEdge 적용
        edge_index_drop, _ = dropout_edge(edge_index, p=self.drop_edge_p, training=self.training)
        x = self.conv2(x, edge_index_drop)
        return x
...
class FullModel(torch.nn.Module):
    def __init__(self, metadata, hidden_channels=32, out_channels=16, dropout=0.1, drop_edge_p=0.2):
        super().__init__()
        self.patent_lin = Linear(384, hidden_channels)
        self.company_lin = Linear(16, hidden_channels)
        
        base_gnn = BaseGNN(hidden_channels, out_channels, dropout, drop_edge_p)
...
for idx, seed in enumerate(SEEDS):
    print(f"\n{'='*50}")
    print(f"[{idx+1}/5] Starting DropEdge Evaluation for Seed: {seed}")
    print(f"{'='*50}")
    ...
    model = FullModel(train_data.metadata(), hidden_channels=HIDDEN_CHANNELS, out_channels=HIDDEN_CHANNELS//2, 
...
print("\n" + "="*50)
print("🌟 Final 5-Seed DropEdge (p=0.2) Results 🌟")
print("="*50)
